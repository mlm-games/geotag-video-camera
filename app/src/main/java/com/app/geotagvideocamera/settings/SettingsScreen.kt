...
private fun DropdownRow(
    ...
    Surface(tonalElevation = 1.dp, shape = MaterialTheme.shapes.medium) {
        ListItem(
            ...
            supportingContent = { Text(entries.getOrNull(selectedIndex) ?: "") },
            trailingContent = { TextButton(enabled = enabled, onClick = { show = true }) { Text(stringResource(R.string.action_select)) } }
        )
    }
    if (show) {
        AlertDialog(
                ...
                Column {
                    entries.forEachIndexed { i, label ->
                        ListItem(
                            ...
                            trailingContent = {
                                RadioButton(selected = i == selectedIndex, onClick = {
                                    onSelected(i); show = false
                                })
                            },
//                            modifier = Modifier.background(MaterialTheme.colorScheme.surfaceDim,RoundedCornerShape(12.dp))
                        )
                    }
                }
            }
